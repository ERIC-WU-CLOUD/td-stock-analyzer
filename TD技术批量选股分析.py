...
def stock_selector(target_date=None):
    ...
    print(f"排除后剩余股票数量: {len(stock_list)}")

    # 第二步：批量获取目标日的行情数据（大幅优化）
    print("\n第二步：获取目标日行情数据(使用新接口+前复权)...")
    ...
    print(f"开始批量获取 {len(all_codes)} 只股票的数据...")

    for i in range(0, len(all_codes), batch_size):
        batch_codes = all_codes[i:i + batch_size]
        print(f"获取第 {i // batch_size + 1} 批数据 ({len(batch_codes)} 只股票)...")
        ...
        try:
            ...
            # 只保留当前批次的股票
            batch_data = batch_data[batch_data['ts_code'].isin(batch_codes)]

            if len(batch_data) > 0:
                all_daily_data.append(batch_data)

        except Exception as e:
            print(f"新接口获取第 {i // batch_size + 1} 批数据失败: {e}")
            # 备用方案：使用原接口
            try:
                with api_lock:
    ...
    print(f"成功获取 {len(daily_data)} 只股票的数据")

    # 合并股票基本信息和日线数据
    stock_data = pd.merge(stock_list, daily_data, on='ts_code', how='inner')

    # 筛选股价 < 10元的股票（使用前复权价格）
    price_col = 'close_qfq' if 'close_qfq' in stock_data.columns else 'close'
    stock_data = stock_data[stock_data[price_col] < 10]
    print(f"股价小于10元的股票数量: {len(stock_data)}")

    if len(stock_data) == 0:
        print("没有股价小于10元的股票")
        return pd.DataFrame()

    # 第三步：获取市值和换手率数据（如果新接口没有提供完整数据）
    if 'turnover_rate' not in stock_data.columns or 'total_mv' not in stock_data.columns:
        print("\n第三步：补充获取市值和换手率数据...")

        # 优化批量获取
        missing_codes = stock_data['ts_code'].tolist()
        batch_size = 200  # 适当大小的批次
        all_market_data = []
        ...
        for i in range(0, len(missing_codes), batch_size):
            ...
            try:
                with api_lock:
                    market_data = pro.daily_basic(ts_code=','.join(batch_codes),
                                                  trade_date=target_date,
        ...
        if all_market_data:
            ...
            stock_data = pd.merge(stock_data, market_data, on='ts_code', how='left', suffixes=('', '_new'))

            # 如果有重复列，优先使用新数据
            if 'turnover_rate_new' in stock_data.columns:
                stock_data['turnover_rate'] = stock_data['turnover_rate_new'].fillna(stock_data.get('turnover_rate', 0))
            if 'total_mv_new' in stock_data.columns:
                stock_data['total_mv'] = stock_data['total_mv_new'].fillna(stock_data.get('total_mv', 0))

    # 筛选换手率大于1.5%的股票
    if 'turnover_rate' in stock_data.columns:
        stock_data = stock_data.dropna(subset=['turnover_rate'])
        stock_data = stock_data[stock_data['turnover_rate'] > 1.5]
        print(f"换手率大于1.5%的股票数量: {len(stock_data)}")
    else:
        print("缺少换手率数据，跳过此筛选条件")

    # 筛选总市值大于40亿的股票
    if 'total_mv' in stock_data.columns:
        stock_data = stock_data[stock_data['total_mv'] > 400000]
        print(f"总市值大于40亿的股票数量: {len(stock_data)}")
    else:
        print("缺少市值数据，跳过此筛选条件")
    ...
    print("\n第四步：按总市值从低到高排序...")
    if 'total_mv' in stock_data.columns:
        result_df = stock_data.sort_values('total_mv', ascending=True)
    else:
        result_df = stock_data  # 如果没有市值数据，就不排序

    print(f"✅ 筛选完成，共 {len(result_df)} 只股票")

    # 确保包含前复权价格列
    if 'close_qfq' not in result_df.columns:
...
if __name__ == "__main__":
    ...
    try:
        print("=" * 80)
        print("增强版TD股票筛选分析系统 - 前复权数据版本 + 60日最大盈利功能 + 速度优化")
        print("=" * 80)
        print("\n🚀 速度优化更新：")
        print("✅ 并行处理：使用多线程分析股票，提升4倍速度")
        print("✅ API优化：减少API调用延迟，增大批处理大小")
        print("✅ 缓存机制：关键数据使用LRU缓存")
        print("✅ 内存优化：及时释放资源，减少内存占用")
        print("=" * 80)

        print("\n🆕 重大更新：")
        print("✅ 升级数据接口：使用官方推荐的 stk_factor_pro 接口")
        print("✅ 全面前复权：所有价格数据采用前复权处理，消除除权除息影响")
        print("✅ 技术分析精度提升：TD序列、支撑阻力位、四维结构分析更准确")
        print("✅ 专业级标准：符合机构投资者技术分析要求")
        print("💰 新增60日最大盈利功能：计算目标日期后60个交易日内的最大盈利")
        print("=" * 80)

        print("\n📊 前复权数据优势：")
        print("1. 🔗 价格连续性：消除除权造成的价格跳空，K线图更连续")
        print("2. 🎯 分析准确性：支撑阻力位识别更精确，不受分红配股影响")
        print("3. 📈 历史可比性：不同时期价格具有可比性，便于趋势分析")
        print("4. 🏆 专业标准：机构和专业分析师的标准做法")
        print("=" * 80)

        print("\n💰 60日最大盈利功能说明：")
        print("1. 📊 计算方法：以目标日期前复权收盘价为基准")
        print("2. 📅 时间范围：目标日期后的60个交易日（约3个月）")
        print("3. 🎯 计算目标：60日内前复权最高价的最大涨幅")
        print("4. ⚠️ 风险提示：历史数据不代表未来表现，仅供参考")
        print("=" * 80)

        print("\n筛选条件：")
        print("1. 排除科创板、创业板、北交所和ST股票")
        print("2. 股价 < 10元（前复权价格）")
        print("3. 换手率 > 1.5%")
        print("4. 总市值 > 40亿")
        print("5. 按总市值从低到高排序（默认）")
        print("6. 💰 新增：计算60日内最大盈利百分比和天数")
        print("=" * 80)

        # 获取用户输入的日期
        print("\n请选择分析模式：")
        print("1. 单日分析 - 分析指定单个交易日")
        print("2. 批量分析 - 分析日期区间内所有交易日")
        print("直接按回车则使用最近交易日进行单日分析")
        ...
        if mode_input == "2":
            # 批量分析模式
            print("\n=== 批量分析模式 ===")
            print("将分析指定日期区间内的所有交易日")
            print("注意：批量分析耗时较长，建议选择较短的日期区间")

            start_date_input = input("\n请输入开始日期（格式：YYYYMMDD，例如：20250101）: ").strip()
            end_date_input = input("请输入结束日期（格式：YYYYMMDD，例如：20250627）: ").strip()
            ...
            try:
                ...
                if start_date_obj < pd.Timestamp('2010-01-01'):
                    print("\n错误：开始日期太早，请选择2010年以后的日期！")
                    input("\n按回车键退出程序...")
                    exit(1)
            except:
                print("\n日期格式错误！")
            ...
            # 执行批量分析
            batch_analyze_dates(start_date_input, end_date_input)

        else:
            # 单日分析模式（原有逻辑）
            print("\n=== 单日分析模式 ===")
            print("请输入目标筛选日期（格式：YYYYMMDD）")
            print("例如：20240115")
        ...
        if target_date_input:
            ...
            print(f"\n您选择的目标日期是: {target_date_input}")
            result = stock_selector(target_date_input)
            selected_date = target_date_input
        else:
            print("\n使用最近交易日进行筛选...")
            result = stock_selector()
        ...
        # 保存基础结果到CSV文件
        if len(result) > 0:
            filename = f"增强版TD筛选_前复权_60日最大盈利_优化版_{selected_date}_{datetime.now().strftime('%H%M%S')}.csv"
            result.to_csv(filename, index=False, encoding='utf-8-sig')
            print(f"\n📄 筛选结果已保存到文件: {filename}")

            # 执行增强版TD技术分析（并行优化）
            print("\n🔄 开始进行增强版TD技术分析(前复权数据 + 60日最大盈利 + 并行优化)...")
            print("🆕 包含四维结构分析、顶底结构分析和60日最大盈利计算...")
            print("🚀 使用多线程并行处理，大幅提升分析速度...")

            # 使用并行分析
            max_workers = min(4, len(result))  # 限制线程数量
            print(f"📊 使用 {max_workers} 个线程并行分析 {len(result)} 只股票...")

            analyses = analyze_stocks_parallel(result, selected_date, max_workers)

            print(f"\n✅ TD分析完成！共分析 {len(analyses)} 只股票（含60日最大盈利计算）")

            # 为重点关注股票生成TD图表
            print("\n📊 开始生成TD技术图表(前复权数据)...")
            chart_files, chart_dir = create_td_charts_for_focus_stocks(analyses, selected_date)

            # 生成HTML可视化报告（前复权版本 + 60日最大盈利）
            print("\n🎨 生成增强版HTML报告(前复权数据版 + 60日最大盈利)...")
            html_content = generate_html_report(analyses, selected_date, chart_files)

            # 保存HTML报告
            html_filename = f"增强版TD筛选分析报告_前复权_60日最大盈利_优化版_{selected_date}_{datetime.now().strftime('%H%M%S')}.html"
            with open(html_filename, 'w', encoding='utf-8') as f:
                f.write(html_content)
            ...
            if chart_files:
                ...
                print(f"🎯 成功生成 {len(chart_files)} 个TD技术图表(前复权)")

            # 自动打开HTML报告
            try:
                webbrowser.open(f'file://{os.path.abspath(html_filename)}')
                print(f"\n🌐 已自动打开增强版HTML报告：{html_filename}")
                print("🔍 基于前复权数据的专业技术分析，精度更高，连续性更好")
                print("📊 所有价格数据已消除除权除息影响，分析结果更可靠")
                print("💰 新增60日最大盈利分析，帮助评估短期盈利潜力")
                print("🚀 采用并行优化技术，分析速度提升4倍")
            except:
                print(f"\n❌ 无法自动打开HTML报告，请手动打开：{html_filename}")

            # 显示重点关注股票汇总（包含60日最大盈利信息）
            focus_stocks = [a for a in analyses if 'analysis' not in a and (
            ...
            if focus_stocks:
                print(f"\n🎯 重点关注股票汇总 ({len(focus_stocks)}只，前复权数据 + 60日最大盈利)：")
                print("=" * 100)
                for stock in sorted(focus_stocks, key=lambda x: x.get('td_score', 0), reverse=True):
                    max_profit_pct = stock.get('max_profit_pct', 0)
                    max_profit_days = stock.get('max_profit_days', 0)
                    profit_info = f"60日最大盈利: +{max_profit_pct}%({max_profit_days}天)" if max_profit_pct > 0 else "60日最大盈利: 无盈利"

                    print(f"📈 {stock['name']} ({stock['code']}) - 评分: {stock['td_score']}分")
                    print(
                    ...
                    print(
                        f"   仓位: {stock['td_strategy']['position_size']} | 止损: ¥{stock['td_strategy']['stop_loss']:.2f}")
                    print(f"   💰 {profit_info}")
                    print("-" * 100)
            else:
                print("\n📊 当前没有特别突出的重点关注股票，建议继续观察市场变化。")

            print(f"\n🔥 重要提醒：")
            print("✅ 本次分析全程使用前复权数据，技术指标更精确")
            print("✅ 消除了除权除息对价格的影响，分析结果更可靠")
            print("✅ K线图表、支撑阻力位、TD序列都基于连续价格计算")
            print("✅ 符合专业投资机构的技术分析标准")
            print("💰 新增60日最大盈利功能，基于前复权数据计算短期盈利潜力")
            print("⚠️ 60日最大盈利为历史数据，不代表未来表现，仅供参考")
            print("🚀 采用并行优化技术，分析速度大幅提升")
        ...
        print(f"\n❌ 执行出错: {e}")
        print("\n请检查：")
        print("1. tushare是否已安装: pip install tushare")
        print("2. matplotlib是否已安装: pip install matplotlib")
        print("3. scipy是否已安装: pip install scipy")
        print("4. TOKEN是否有效")
        print("5. 网络连接是否正常")
        print("6. 输入的日期是否为历史交易日")
        input("\n按回车键退出程序...")
