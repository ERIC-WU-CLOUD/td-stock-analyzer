...
def main():
    ...
    with tab4:
        ...
        ### 🔬 核心技术分析功能
        - **TD序列分析**: Setup、Countdown、Combo完整计算
        - **TDST动态支撑阻力**: 基于TD序列的动态价位
        - **四维结构分析**: 时间、价格、成交量、空间四个维度
        - **顶底结构识别**: 双顶双底、头肩形态等经典反转结构
        - **增强版成交量分析**: 量价配合度、异常量能检测
        - **支撑阻力位计算**: 多层次关键价位识别
        - **交互式图表**: Plotly高质量可视化图表
        ...
        ### 🔢 TD Setup (设置)
        - **计算原理**: 比较当前收盘价与4天前收盘价
        - **买入Setup**: 连续收盘价低于4天前收盘价，计数1-9
        - **卖出Setup**: 连续收盘价高于4天前收盘价，计数1-9
        - **完美设置**: Setup 8或9时，当日最低/最高价突破前两日极值
        ...
        ### ⏰ TD Countdown (倒计时)
        - **启动条件**: Setup 9完成后启动
        - **买入倒计时**: 收盘价≤2天前最低价，计数1-13
        - **卖出倒计时**: 收盘价≥2天前最高价，计数1-13
        - **13完成**: 倒计时结束，确认反转信号
        ...
        ### 🔄 TD Combo (组合)
        - **启动条件**: Setup 9完成同时启动
        - **计算方式**: 收盘价持续低于/高于前一日收盘价
        - **重要意义**: 提供额外的反转确认信号
        
        ### 🎯 TDST支撑阻力位
        - **动态计算**: 基于Setup完成时的价格极值
        - **TDST支撑**: 买入Setup 9完成期间的最低价
        - **TDST阻力**: 卖出Setup 9完成期间的最高价
        - **实用价值**: 比传统支撑阻力位更精确
        ...
        ### 🎯 评分构成要素
        1. **TD Setup权重** (35分)：Setup 9=35分，Setup 7-8=20分
        2. **TD Countdown权重** (45分)：Countdown 13=45分，接近完成=30分
        3. **完美设置加分** (20分)：提高信号可靠性
